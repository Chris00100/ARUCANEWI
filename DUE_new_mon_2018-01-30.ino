...
// BMP180/280 Pressure sensor, lib from https://github.com/adafruit/Adafruit-BMP085-Library
// or https://github.com/adafruit/Adafruit_BMP280_Library
  #include <Wire.h>                // i2c lib from Arduino
  #include <Adafruit_BMP085.h>
  //#include <Adafruit_BMP280.h>
  #include <Adafruit_Sensor.h>     // Common sensor lib
  Adafruit_BMP085 bmp;
  //Adafruit_BMP280 bmp;           // Use default I2C interface
...
// Announce version
  ...
  lcd.clear(); lcd.print(versionString); delay(1500);
    
//BMP180 init
    Serial.println("BMP180/280 air pressure sensor init");
    if (!bmp.begin()) {
      BMP180Exist = 0;
      Serial.println("0 BMP180 pressure");
      lcd.clear(); lcd.print("0 BMP180 press"); delay(1500); 
    }
    else {
      BMP180Exist = 1;
      Serial.println("1 BMP180 press");
      lcd.clear(); lcd.print("1 BMP180 press"); delay(1500); 
    }
...
//DHT11 Init
    ...
    if (isnan(h) || isnan(t)) {
      lcd.clear(); lcd.print("0 DHT humidity"); delay(1500);
      DHT11Exist = 0; 
    }
    else {
      DHT11Exist = 1;
      lcd.clear(); lcd.print("1 DHT humidity"); delay(1500); 
    }
    
// Dallas Init
    Serial.println("Dallas tmp sensors init");
    lcd.clear(); lcd.print("Temp sensors..."); 
    ...
    Serial.print(numberOfDevices); Serial.println(" sensor heating");
    ...
    Serial.print(numberOfDevices); Serial.println(" sensor outside");
    ...
    Serial.print(numberOfDevices); Serial.println(" sensor engine");
    ...
    Serial.print(numberOfDevices); Serial.println(" sensor fridge");
    ...
    delay(1200); lcd.clear(); lcd.print("Ready, starting"); delay(700); lcd.clear();
...
void SendDHT11BMP180() {
  ...
  if ( Updated+TempHumidityReadPeriod<millis() ) {
    ...
    tN2kMsg N2kMsg;
    if (DHT11Exist) DHT11OutsideHumidity = dht.readHumidity();
    if (BMP180Exist) BMP180AirPressure = bmp.readPressure();
    ...
    Serial.print("  DHT11 CabinTemperature: "); Serial.print(DHT11MainCabinTmp);Serial.println(" *C ");
  }
}
...
void DisplayOn2x16() { 
  ...
  if ( DispUpdated+DispUpdatePeriod<millis() ) {
    DispUpdated=millis();
    lcd.clear(); lcd.setCursor(0, 0); lcd.print(DispBlock);
    ...
    if ( DispBlock >= 2 and DispBlock <= 4 ){
    ...
    if ( DispBlock > 4 and DispBlock <= 6 ){
      lcd.clear(); lcd.setCursor(0, 0); lcd.print("Cabin Tmp");
      if (DHT11Exist) {
        lcd.setCursor(11, 0); 
        lcd.print(DHT11MainCabinTmp);
      }
